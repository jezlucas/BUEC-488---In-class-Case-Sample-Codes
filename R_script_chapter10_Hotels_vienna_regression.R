...
# You might also create the Table here
ht<-huxreg(reg0,reg1, reg2,
...
stargazer_r(list(reg1, reg2, reg3, reg4), se = 'robust', digits=3, out=paste(output,"T10_hotels2_R.doc",sep=""))
...
hotels %>%
  ...
  .[1:5,] %>%
  as.data.frame() %>% 
  stargazer(summary= FALSE, digits = 1, out = paste(output,"T10_hotels_best_deals.html",sep=""))
